<div class="col-md-3 col-sm-4">
	...
	<ul class="usernavdash">
		...
		<li class="active"><a href="{{route('home')}}"><i class="fas fa-tachometer-alt" aria-hidden="true"></i> {{__('Dashboard')}}</a>
		</li>
		...
		<li><a href="{{route('my.messages')}}"><i class="fas fa-envelope" aria-hidden="true"></i> {{__('My Messages')}}<span class="badge">@if(Auth::user()->countApplicantMessages() != 0){{__(Auth::user()->countApplicantMessages())}}@endif</span></a>
		</li>

		<li><a href="{{route('my.followings')}}"><i class="fa fa-user" aria-hidden="true"></i> {{__('My Followings')}}</a>
		</li>

		<li><a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();"><i class="fa fa-sign-out-alt" aria-hidden="true"></i> {{__('Logout')}}</a>
